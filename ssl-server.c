	...
	// is complete.
	  ...
	  switch(buffer[0]) {
	  	...
	  	case 'C':
		    ...
		    // Create a key-value pair to insert in the database. Must specify the size
 			// of each datum in bytes.
		    datum key = {titlePtr, strlen(titlePtr)};
  			datum value = {values, strlen(values)};
